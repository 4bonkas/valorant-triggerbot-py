...
system("mode 80,18 & title triggerbot & powershell $H=get-host;$W=$H.ui.rawui;$B=$W.buffersize;$B.width=80;$B.height=9999;$W.buffersize=$B;")
...
def exit_():
    system("echo press a key stupid whore... & pause >nul")
    _exit(0)
...
try:
    ...
    print(f"{INFO} installing ur modules because your fatass doesn't know how.")
...
try:
    ...
    print(f"{SUCCESS} hk: {TRIGGER}\n{SUCCESS} color: {HIGHLIGHT}\n")
except (FileNotFoundError, ValueError):
    print(f"{ERROR} WHERE YO CONFIG AT?\n")
    HIGHLIGHT = input(f"{INFO} color\n\n[\x1b[35m1\x1b[38;5;255m] red\n[\x1b[35m2\x1b[38;5;255m] purple\n\n> ")
    if HIGHLIGHT not in ["1", "2"]:
        print(f"{ERROR} 1 or 2?\n")
        exit_()
    if HIGHLIGHT == "1":
        HIGHLIGHT = "red"
    elif HIGHLIGHT == "2":
        HIGHLIGHT = "purple"
    print(f"\n{SUCCESS} color was saved to config.txt\n{INFO} write your hk in config.txt\n")
    with open("config.txt", "w") as f:
        f.write(f"replace the first line with your hotkey  e.g.  c   or   `   or even   ctrl + alt + z\n{HIGHLIGHT}")
    exit_()
